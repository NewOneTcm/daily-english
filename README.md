...
阅读任务里，**点原文里的任何单词**（don't 这类带撇号的也算一个），就把它和出处句子一起收进生词库，然后走完整的刻意练习流程：

1. **语境解释**：点「AI 解释（结合原句）」，AI 按这个词在原句里的意思给词性+释义+新例句；也可以自己写一条
2. **造句练习**：用这个词造一句自己的话；「检查造句」会先验证句子里真的用到了这个词（含变形），再跑一遍本地语法点评
3. **AI 点评造句**：深度复盘你造的句子
4. **存入学习库**：生成「看释义回忆你的造句」复习卡（带原句语境），进入间隔复习队列，可随表达库一键导出 SuperMemo

生词列表在「表达库」页顶部，没造句/没入库的可以随时回去补。
